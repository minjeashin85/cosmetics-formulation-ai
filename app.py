...
# ------------------------------------------------------------------
    ...
    # Python-based base URL detection as fallback
    base_url = ""
    try:
    ...
    for t in FORMULATION_TYPES:
        grid_html += f'''
        <div class="cfa-card-link" onclick="selectType('{t["id"]}')">
    ...
    <head>
      ...
      <style>
        ...
        .cfa-card-link:hover .liquid-glass {{
          background: linear-gradient(135deg, rgba(255,255,255,0.1), rgba(255,255,255,0.03));
          border-color: rgba(255,255,255,0.22);
          box-shadow: 0 30px 70px rgba(0,0,0,0.65), inset 0 1px 1px rgba(255,255,255,0.3);
          transform: translateY(-5px);
        }}

        .cfa-card-link:hover .cfa-icon-wrap {{
          transform: scale(1.18) rotate(-4deg);
          filter: drop-shadow(0 0 25px rgba(6,182,212,0.45));
        }}

        .cfa-card-link:hover .cfa-name-pill {{
        ...
        .cfa-card-link:hover .cfa-body-anim {{ animation: cfaWobble .8s ease-in-out; transform-box: fill-box; transform-origin: 50% 100%; }}
        .cfa-card-link:hover .cfa-cap-anim {{ animation: cfaCapPop .8s ease-in-out; transform-box: fill-box; transform-origin: 50% 100%; }}
        .cfa-card-link:hover .cfa-bubble-anim {{ animation: cfaBubbleUp 1.2s ease-in-out infinite; transform-box: fill-box; transform-origin: 50% 100%; }}
        .cfa-card-link:hover .cfa-shine-anim {{ animation: cfaShineSweep .95s ease-in-out infinite; }}
      </style>
      
      <script>
        function selectType(ftype) {{
          let parentUrl = "{base_url}";
          ...
          if (parentUrl) {{
            ...
            window.parent.location.href = "/?ftype=" + ftype;
          }}
        }}
      </script>
    </head>
    <body>
      <div class="cfa-type-grid">
        {grid_html}
      </div>
    </body>
    </html>
